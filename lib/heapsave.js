...
HeapSave = {
  toS3: function(key, secret, bucket, interval) {
    ...
    s3PutFile = Meteor.wrapAsync(s3Client.putFile.bind(s3Client));
...
takeDump = Meteor.wrapAsync(function (name, callback) {
  var filename = '/tmp/' + name + '.heapsnapshot';
...
compressIt = Meteor.wrapAsync(function(filename, callback) {
...
deleteFile = Meteor.wrapAsync(fs.unlink);
